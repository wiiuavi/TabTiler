document.addEventListener('DOMContentLoaded', () => {
  ...
  const closeOriginalTabsToggle = document.getElementById('closeOriginalTabsToggle')
  
  chrome.storage.local.get(['keep100Zoom', 'hideScrollbars', 'closeOriginalTabs'], (result) => {
    keep100Toggle.checked = result.keep100Zoom || false
    hideScrollbarsToggle.checked = result.hideScrollbars !== false
    closeOriginalTabsToggle.checked = result.closeOriginalTabs !== false
  })
  ...
  closeOriginalTabsToggle.addEventListener('change', (e) => {
    chrome.storage.local.set({ closeOriginalTabs: e.target.checked })
  })
